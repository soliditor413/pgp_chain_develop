...
For more information on how to encrypt and decrypt from the command line, reference the README: https://github.com/pgprotocol/pgp-chain/ChainBridge
...
func KeypairFromAddress(chainType, path string, password []byte, insecure bool) (crypto.Keypair, error) {
	...
	kp, err := ReadFromFileAndDecrypt(path, password, keyMapping[chainType])
	if err != nil {
		return nil, err
	}
