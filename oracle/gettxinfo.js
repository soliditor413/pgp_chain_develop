        ...
        if (txreceipt.status) {
            for (const log of logs) {
                if (log.address === common.contract.options.address && log.topics[0] === common.payloadReceived.signature) {
                    ...
                    // let crosschainamount = String(common.retnum(common.web3.utils.fromWei(event["_crosschainamount"], "gwei") * 10));
                    // let outputamount = String(common.retnum(common.web3.utils.fromWei(event["_amount"], "gwei") * 10));
                    let crosschainamount = (event["_crosschainamount"]/1e8).toString();
                    let outputamount = (event["_amount"]/1e8).toString();
