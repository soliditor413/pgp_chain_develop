// Copyright 2019 The pgp-chain Authors
...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches pgp in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches pgp in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func (e *NoRewardEngine) GetCurrentProducers() [][]byte {
	return [][]byte{}
}

func (e *NoRewardEngine) GetValidatorsByHeight(height uint64) ([][]byte, uint8, uint64, error) {
	validators := e.GetCurrentProducers()
...
func (api *RetestethAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	...
	blockchain, err := core.NewBlockChain(ethDb, nil, chainConfig, engine, engine, vm.Config{}, nil)
	if err != nil {
		return false, err
	}
