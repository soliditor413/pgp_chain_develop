...
var (
	...
	ethPortFlag = flag.Int("ethport", 20668, "Listener port for the devp2p connection")
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/pgprotocol/pgp-chain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			signed, err := f.keystore.SignTx(f.account, tx, f.config.GetChainIDByHeight(f.head.Number))
			if err != nil {
				f.lock.Unlock()
				if err = sendError(conn, err); err != nil {
