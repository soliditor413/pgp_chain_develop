...
import (
	...
	"github.com/pgprotocol/pgp-chain/accounts/external"
	"github.com/pgprotocol/pgp-chain/accounts/keystore"
	"github.com/pgprotocol/pgp-chain/accounts/scwallet"
	"github.com/pgprotocol/pgp-chain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of pgp is "pgp". If no
...
// These resources are resolved differently for "pgp" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by pgp 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'pgp' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "pgp-chain-keystore")
		ephemeral = keydir
	}
