// Copyright 2022 The pgp-chain Authors
...
import (
	...
	jsassets "github.com/pgprotocol/pgp-chain/eth/tracers/js/internal/tracers"
)
...
func (t *jsTracer) CaptureStart(env *vm.EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	...
	if env.Context.Time == nil {
		env.Context.Time = big.NewInt(0)
	}
	rules := env.ChainConfig().Rules(env.Context.BlockNumber, env.Context.Random != nil, env.Context.Time.Uint64())
